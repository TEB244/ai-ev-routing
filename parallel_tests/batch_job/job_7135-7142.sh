#!/bin/bash
#SBATCH --job-name=Exp_parallel_train_7135_7142-GPUH100
#SBATCH --output=parallel_tests/batch_train_7135-7142/output.log
#SBATCH --error=parallel_tests/batch_train_7135-7142/error.log
...
# ─── Configuration ────────────────────────────────────────────────────────────
EXPERIMENTS=(7135 7142)
DATA_DIR="/home/sgomez/scratch/metrics/Exp"
...
for EXP_ID in "${EXPERIMENTS[@]}"; do
    ...
    python main.py -g 0 -e "${EXP_ID}" -d "${DATA_DIR}" \
        > "${LOGS_DIR}/Exp_${EXP_ID}/output.log" \
        2> "${LOGS_DIR}/Exp_${EXP_ID}/error.log" &
    PIDS+=($!)
